...
app.post('/api/frete', async (req, res) => {
    ...
    try {
        ...
        let basePac = 0, prazoPac = 0;

        if (uf === 'ES') { basePac = 15.00 + (peso * 2.50); prazoPac = 3; } 
        else if (['SP', 'RJ', 'MG'].includes(uf)) { basePac = 25.00 + (peso * 4.00); prazoPac = 6; } 
        else if (['PR', 'SC', 'RS', 'DF', 'GO', 'BA'].includes(uf)) { basePac = 38.00 + (peso * 6.50); prazoPac = 9; } 
        else { basePac = 55.00 + (peso * 10.00); prazoPac = 14; }

        const valorSedex = basePac + 25.00 + (peso * 4.00);
        const prazoSedex = Math.max(1, prazoPac - 4); 

        res.json({ 
            success: true, 
            pac: { Valor: basePac.toFixed(2).replace('.', ','), PrazoEntrega: prazoPac.toString() }, 
            sedex: { Valor: valorSedex.toFixed(2).replace('.', ','), PrazoEntrega: prazoSedex.toString() } 
        });
